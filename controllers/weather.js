...
const handleGetWeather = async (req, res) => {
  ...
  try {
    const data = await axios.get(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&exclude=minutely,hourly,alerts,current&units=metric&appid=${process.env.ONEWS_OPENWEATHER_KEY}`
    );
